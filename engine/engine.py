...
def create_train_engine(model, optimizer, non_blocking=False):
    ...
    def _process_func(engine, batch):
        ...
        loss, metric = model(data, labels,train_item = 1,
                             cam_ids=cam_ids,img_paths = img_paths,
                             epoch=epoch)
...
def create_eval_engine(model, non_blocking=False):
    ...
    def _process_func(engine, batch):
        ...
        with no_grad():
            feat = model(data, train_item=0, cam_ids=cam_ids.to(device, non_blocking=non_blocking), img_paths=img_paths)
